...
func circle2circleQuery(p1, p2 Vect, r1, r2 Float, con *Contact) int {
	...
	dist := Float(math.Sqrt(float64(distSqr)))

	pDist := dist
	if dist == 0.0 {
		pDist = Float(math.Inf(1))
	}
...
func segmentEncapQuery(p1, p2 Vect, r1, r2 Float, con *Contact, tangent Vect) int {
...
func findMSA(poly *PolygonShape, axes []PolygonAxis, num int) (min_out Float, min_index int) {
...
func findVerts(contacts []*Contact, poly1, poly2 *PolygonShape, n Vect, dist Float) int {
	num := 0
...
func findVertsFallback(contacts []*Contact, poly1, poly2 *PolygonShape, n Vect, dist Float) int {
	num := 0
...
func findPoinsBehindSeg(contacts []*Contact, num *int, seg *SegmentShape, poly *PolygonShape, pDist, coef Float) {
